...
jest.mock('../../stores/facetsStore', () => ({
  ...jest.requireActual('../../stores/facetsStore'),
...
describe('CheckboxFacet', () => {
  ...
  describe('when required search is not found', () => {
    it('should render "No matching options" message', () => {
      getSearchTerm.mockReturnValue('test search');
      renderCheckboxFacet();
  ...
  describe('when `hideMoreButton` is added', () => {
    it('should display all options', () => {
      const { queryByText, getByText } = renderCheckboxFacet({
        hideMoreButton: true,
      });

      expect(queryByText('button', { name: 'More' })).not.toBeInTheDocument();
      expect(getByText(defaultProps.dataOptions[5].label)).toBeInTheDocument();
    });
  });
});
