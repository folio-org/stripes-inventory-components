...
function withFacets(WrappedComponent) {
  class FacetsHoc extends React.Component {
    ...
    getCqlQuery = (isBrowseLookup, query, queryIndex, data, filterConfig) => {
      if (isBrowseLookup) {
      ...
      return buildQuery(query, {}, { ...data, query }, { log: () => null }, filterConfig) || '';
    }

    fetchFacets = ({ data, isBrowseLookup, filterConfig }) => async (properties = {}) => {
      const {
        onMoreClickedFacet,
        focusedFacet,
        accordions,
      ...
      const cqlQuery = this.getCqlQuery(isBrowseLookup, query, queryIndex, data, filterConfig);
