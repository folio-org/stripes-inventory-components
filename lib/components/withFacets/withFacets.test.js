...
const filterConfig = {
  indexes: [
    { label: '.title', value: 'title', queryTemplate: 'title all "%{query.query}"' },
  ],
  filters: [
    { name: 'resource', cql: 'instanceTypeId', values: [] },
    { name: 'searchContributors', cql: 'contributors.contributorNameTypeId', values: [] },
    { name: 'format', cql: 'instanceFormatIds', values: [] },
  ],
};
...
const WrappedComponent = ({
  ...
  return (
    <button
      type="button"
      onClick={() => fetchFacets({ data, isBrowseLookup, filterConfig })(properties)}
    >
      fetchFacetsButton
    </button>
  );
