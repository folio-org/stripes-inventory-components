...
const useCommonData = () => {
  ...
  const { locationsFromAllTenants, isLoading: isLocationsOfAllTenantsLoading } = useLocationsFromAllTenantsQuery({ tenantIds });
  const { locations, isLoading: isLocationsLoading } = useLocationsQuery();
  ...
  const isCommonDataLoading = isLocationsLoading || isConsortiaTenantsLoading || isInstanceTypesLoading
    || isLocationsOfAllTenantsLoading || isMaterialTypesLoading || isNatureOfContentTermsLoading
