...
const useFacetOptions = ({ activeFilters, qindex, isBrowseLookup, data, facetsData, initialAccordionStates }) => {
  const intl = useIntl();
  ...
  const optionsMap = useMemo(() => {
    return {
      ...
      [FACETS_CQL.INSTANCE_TYPE]: (values) => getFacetOptions(activeFilters[FACETS.RESOURCE], values, data.resourceTypes),
      ...
      [FACETS_CQL.ITEMS_STATUSES]: (values) => getItemStatusesOptions(activeFilters[FACETS.ITEM_STATUS], values, data.itemStatuses, intl),
  ...
  const defaultOptions = useMemo(() => {
    return Object.keys(initialAccordionStates).reduce((acc, name) => {
      const facetCql = FACETS_TO_REQUEST[name];
      acc[facetCql] = [];
      return acc;
    }, {});
  ...
  }, [facetsData, defaultOptions, optionsMap]);
