...
const useFacetOptions = ({ activeFilters, qindex, isBrowseLookup, data, facetsData, initialAccordionStates }) => {
  ...
  const optionsMap = useMemo(() => {
    return {
      ...
      [FACETS_CQL.SUBJECT_SOURCE]: (values) => getFacetOptions(activeFilters[FACETS.SUBJECT_SOURCE], values, data.subjectSources),
      [FACETS_CQL.SUBJECT_TYPE]: (values) => getFacetOptions(activeFilters[FACETS.SUBJECT_TYPE], values, data.subjectTypes),
