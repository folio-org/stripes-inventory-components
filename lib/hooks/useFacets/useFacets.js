...
const useFacets = ({
  ...
  const handleToggleAccordion = useCallback(({ id: name }) => {
    ...
    if (!facetState.isOpen) {
      ...
      const facetCql = getFacetCql(name)[FACETS_FETCH_ALL.includes(name) ? 'all' : 'default'];
  ...
  useEffect(() => {
    ...
    const cqlOfOpenFacets = facetStates
      ...
      .map(facet => {
        ...
        // some facets don't have +More button so we need to fetch all values at once
        return getFacetCql(facet.name)[FACETS_FETCH_ALL.includes(facet.name) ? 'all' : 'default'];
      })
      .join(',');
