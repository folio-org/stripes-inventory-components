...
describe('useFacets', () => {
  ...
  describe('when options are selected in different facets and more facets are opened', () => {
    it('should respect all of them', async () => {
      ...
      expect(mockGet).toHaveBeenLastCalledWith('search/instances/facets', {
        searchParams: {
          facet: 'items.effectiveLocationId,staffSuppress,shared,languages',
          query: '(isbn="test") and staffSuppress=="false" and items.effectiveLocationId=="id-1"',
        },
      });
      expect(result.current.activeFilters).toEqual({
  ...
  describe('when several facets are open and user selects an option of any facet', () => {
    it('should fetch options for all open facets', async () => {
      ...
      expect(mockGet).toHaveBeenLastCalledWith('search/instances/facets', {
        searchParams: {
          facet: 'items.effectiveLocationId,staffSuppress,languages',
  ...
  describe('when user does not have staff suppress facet permission', () => {
    describe('and we are in Browse mode', () => {
      it('should not apply staffSuppress.false facet by default', async () => {
