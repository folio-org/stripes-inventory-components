...
export const getCqlQuery = (isBrowseLookup, query, data, isNewCallNumberBrowseAvailable) => {
  const { qindex } = query;
  ...
  if (isBrowseLookup) {
    ...
    if (isNewCallNumberBrowseAvailable && isCallNumber) {
  ...
  return buildQuery(query, {}, { query }, { log: () => null }) || '';
};
