...
export const getTemplateForSelectedFromBrowseRecord = (queryParams, queryIndex, queryValue) => {
  ...
  if (queryIndex === queryIndexes.CONTRIBUTOR) {
    const escapedQueryValue = escapeCqlValue(queryValue);
...
export const processSearchErrors = ({ error, callout, intl, pathname, host, params, setIsRequestUrlExceededLimit = noop }) => {
  let message = intl.formatMessage({ id: 'stripes-inventory-components.error.badRequest' });

  if (error.httpStatus === HttpStatusCodes.ERROR_URI_TOO_LONG
    // This condition is to handle CORS error, that occurs after exceeding the request URL limit.
    || getRequestUrlLength(pathname, host, params) >= REQUEST_URL_LIMIT) {
    setIsRequestUrlExceededLimit(true);
    message = intl.formatMessage({ id: 'stripes-smart-components.error.requestUrlLimit' }, { limit: REQUEST_URL_LIMIT });
  } else {
